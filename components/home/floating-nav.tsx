...
export function FloatingNav() {
    ...
    return (
        <nav className="fixed top-5 right-16 z-50 hidden md:flex items-center gap-0 bg-[var(--color-navy)] rounded-full px-1.5 py-1 shadow-[0_4px_12px_rgba(255,255,255,0.15)] ring-2 ring-white/20">
            {routes.map((route) => {
...
export function MobileNav() {
    ...
    return (
        <nav className="fixed top-0 left-0 right-0 z-50 md:hidden flex items-center gap-0 bg-[var(--color-navy)] px-4 py-3 overflow-x-auto shadow-[0_2px_8px_rgba(255,255,255,0.1)] border-b-2 border-white/20 scrollbar-hide">
            {routes.map((route) => {
                const Icon = route.icon;
                const isActive = pathname === route.path;
